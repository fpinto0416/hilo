# -*- coding: utf-8 -*-
"""
Walk-Forward HiLo — VERSAO 4 HORAS
selecao do hilo de MAIOR PNL ACUMULADO ate a data, rebalanceamento ANUAL

O QUE MUDA EM RELACAO A VERSAO DIARIA
- entrada: parquet de "database_4h" com DatetimeIndex TZ-AWARE (gerado pelo
  coletor tvDatafeed), nao xlsx com index de datas.
- BARRAS_ANO deixa de ser 252. O pregao da B3 rende ~2 a 2,5 barras de 4h por
  dia, entao o ano tem ~500-630 barras. Todo Sharpe, volatilidade e retorno
  anualizado depende disso; usar 252 aqui infla a vol anualizada em ~1,4x e
  destroi a comparabilidade com a versao diaria.
- as janelas moveis passam a ser declaradas em DIAS DE CALENDARIO e
  convertidas para barras internamente, senao "janela de 250" viraria 250
  barras = ~4 meses em vez de um ano.
- hilo_min/hilo_max sao em BARRAS. Para comparar com a versao diaria, use a
  equivalencia em tempo: hilo 50 diario ~ hilo 100-125 em 4h. Comparar
  "hilo 50" nos dois timeframes NAO e o mesmo teste — em 4h isso seria um
  lookback de ~22 pregoes contra 50.

LIMITACAO DE DADOS: o TradingView corta o historico intraday. Espere 2 a 4
anos de 4h, ou seja 1.000 a 2.500 barras — contra ~6.500 barras diarias
(26 anos) da versao diaria. Isso reduz drasticamente o numero de janelas
INDEPENDENTES e, portanto, o poder dos testes. O script imprime o historico
efetivo de cada ativo no log.
...
# PARAMETROS
# =========================
# ---- CALIBRACAO DO TIMEFRAME ----
# barras por pregao no timeframe usado. Detectado automaticamente da base
# (mediana de barras por dia) e usado para converter dias em barras.
# Pode ser fixado via HILO_BARRAS_DIA se a deteccao vier estranha.
_bpd = os.environ.get("HILO_BARRAS_DIA", "").strip()
BARRAS_POR_DIA = float(_bpd) if _bpd else None      # None = detectar
PREGOES_ANO = 252
BARRAS_ANO = None                                   # preenchido apos deteccao

# janelas moveis em DIAS DE CALENDARIO (convertidas para barras depois)
janelas_dias = [int(x) for x in
                os.environ.get("HILO_JANELAS_DIAS", "30,60,90,180,250").split(",")]

montante_inicial = 10000
# hilo em BARRAS de 4h. Para equivaler ao hilo diario, multiplique pelo numero
# de barras por pregao: hilo 50 diario ~ hilo 100-150 em 4h. Comparar "hilo 50"
# nos dois timeframes compara lookbacks de tamanhos diferentes.
hilo_max = int(os.environ.get("HILO_MAX", "200"))
hilo_min = int(os.environ.get("HILO_MIN", "4"))
# so comeca a selecionar o melhor hilo apos esta quantidade de barras (pregoes)
# EM BARRAS. 1000 barras de 4h ~ 450 pregoes ~ 1,8 ano, contra 1000 pregoes
# (~4 anos) da versao diaria. Com historico intraday curto, 1000 barras ja
# consomem boa parte da amostra — reduza se o log acusar muitos descartes.
dias_minimos_selecao = int(os.environ.get("HILO_DIAS_MIN", "1000"))
...
# preenchido apos a deteccao de BARRAS_POR_DIA (ver bloco de calibracao):
# janelas_dias sao convertidas para BARRAS aqui.
janelas_carteira = None

# minimo de barras de historico para um hilo ser elegivel na selecao
# em BARRAS (nao dias). Default: 1 ano de barras, definido apos a deteccao.
barras_minimas_hilo = None
...
# EM PREGOES, convertido para barras apos detectar BARRAS_POR_DIA.
# Declarar em pregoes evita o erro de usar os mesmos numeros da versao diaria:
# "50,70,90" em BARRAS de 4h seria 25/35/45 pregoes, nao 50/70/90.
hilos_fixos_pregoes = [float(x) for x in
    os.environ.get("HILO_FIXOS_PREGOES", "20,36,50,64").split(",")]
hilos_fixos_janelas = None   # preenchido no bloco de calibracao
...
dados_base = os.environ.get("HILO_DADOS_BASE", r"C:\AI_Sandbox\projeto_MIA\projeto_MIA26\database_4h")
TZ_ALVO = os.environ.get("HILO_TZ", "America/Sao_Paulo")
arquivo_saida = os.environ.get("HILO_ARQUIVO_SAIDA", "saidas_hilo_4h.xlsx")
...
def annualize_return(total_return, n_bars, bars_per_year=None):
    if bars_per_year is None:
        bars_per_year = BARRAS_ANO
    if n_bars <= 0:
        return np.nan
    base = 1 + total_return
    if base <= 0:
...
def preparar_dados(caminho_arquivo):
    if str(caminho_arquivo).lower().endswith(".parquet"):
        df = pd.read_parquet(caminho_arquivo)
    else:
        df = pd.read_excel(caminho_arquivo, index_col=0, parse_dates=True)

    df.index = pd.DatetimeIndex(df.index)
    # o resto do pipeline compara datas entre ativos; se um vier tz-aware e
    # outro naive, a comparacao quebra. Normaliza tudo para naive no fuso alvo.
    if df.index.tz is not None:
        df.index = df.index.tz_convert(TZ_ALVO).tz_localize(None)
...
def rodar_walkforward_anual(ticker, pnl_por_hilo_df, ops_por_hilo):
    ...
    def _stats_hilo(col_hilo, n_i, t_val, pos):
        ...
        return {
            ...
            "n": nn,
            "sharpe": (float(tt / np.sqrt(nn) * np.sqrt(BARRAS_ANO))
    ...
    for data_sel in datas_selecao:
        ...
        sharpe_escolhido = float(t_escolhido / np.sqrt(n_escolhido) * np.sqrt(BARRAS_ANO))
    ...
    if not np.isfinite(base_wf) or base_wf <= 0:
        ...
        rent_aa = annualize_return(rent_total, n_investido, BARRAS_ANO) if rent_total > -1 else np.nan
    ...
    vol_aa = float(ret_investido.std() * np.sqrt(BARRAS_ANO))
    ret_aa_simples = float(ret_investido.mean() * BARRAS_ANO)
        ...
        for K in tamanhos_janelas:
            ...
            for _ in range(n_simulacoes_janelas):
                ...
                if not np.all(np.isfinite(curva)) or np.any(curva <= 0):
                    descartadas += 1
                    continue
                geo.append(curva[-1] ** (BARRAS_ANO / n) - 1.0)
...
def _teste_pareado(ra, rb, rot_a, rot_b, hilo, n_boot):
    ...
    return {
        ...
        "N_Barras": n,
        "Dif_Media_AA_pp": float(np.mean(dif) * BARRAS_ANO * 100),
        "Vol_Dif_AA": sd * np.sqrt(BARRAS_ANO) * 100,
        "t_Padrao": t,
...
def _metricas_serie(ret, curva, frac_investido, rot_ativo, rot_estrat, hilo):
    ...
    linha = {
        ...
        "Rent_AA_Geometrica": (float(curva[-1] / montante_inicial) ** (BARRAS_ANO / n) - 1.0) * 100
                              if curva[-1] > 0 else np.nan,
        "Vol_AA": sd * np.sqrt(BARRAS_ANO) * 100,
        "Sharpe_rf0": float(np.mean(ret) / sd * np.sqrt(BARRAS_ANO)) if sd > 0 else np.nan,
...
def montar_longonly_vs_bh(series_por_hilo):
    ...
    cdi_diario = (1.0 + cdi_aa_longonly / 100.0) ** (1.0 / BARRAS_ANO) - 1.0
    ...
    for hilo in sorted(series_por_hilo):
        ...
        for K in tamanhos_carteira_longonly:
            ...
            for estrat, chave in [("LongShort", "long_short"), ("LongOnly", "long_only"),
                ...
                n = R.shape[1]
                geo = curva[:, -1] ** (BARRAS_ANO / n) - 1.0
                vol = R.std(axis=1, ddof=1) * np.sqrt(BARRAS_ANO)
...
def _detectar_barras_por_dia(tickers, pasta):
    """
    Mede a mediana de barras por pregao numa amostra de ativos.

    Nao chuta 2: com ancora em 9h o pregao pode render 2 ou 3 barras conforme
    o horario de verao e o leilao de fechamento, e a diferenca entre 2,0 e 2,5
    muda a volatilidade anualizada em 12%.
    """
    amostras = []
    for t in tickers[:12]:
        for ext in (".parquet", ".xlsx"):
            cam = os.path.join(pasta, f"{t}{ext}")
            if not os.path.exists(cam):
                continue
            try:
                d = (pd.read_parquet(cam) if ext == ".parquet"
                     else pd.read_excel(cam, index_col=0, parse_dates=True))
                idx = pd.DatetimeIndex(d.index)
                if idx.tz is not None:
                    idx = idx.tz_convert(TZ_ALVO).tz_localize(None)
                por_dia = pd.Series(1, index=idx).resample("D").sum()
                por_dia = por_dia[por_dia > 0]
                if len(por_dia) > 50:
                    amostras.append(float(por_dia.median()))
            except Exception:
                pass
            break
    if not amostras:
        # NAO cair em 2.0 silenciosamente: sem arquivo lido, qualquer
        # BARRAS_ANO e chute, e todo Sharpe/vol/retorno anualizado sai errado
        # sem nenhum sintoma visivel. Melhor parar.
        raise FileNotFoundError(
            f"Nenhuma serie legivel em: {os.path.abspath(pasta)}\n"
            f"  existe? {os.path.isdir(pasta)}\n"
            f"  arquivos .parquet/.xlsx na pasta: "
            f"{len([f for f in os.listdir(pasta) if f.endswith(('.parquet', '.xlsx'))]) if os.path.isdir(pasta) else 0}\n"
            f"  primeiros tickers procurados: {tickers[:5]}\n"
            "Ajuste dados_base (ou a variavel de ambiente HILO_DADOS_BASE) "
            "para a pasta 'database_4h' criada pelo coletor. O coletor grava em "
            "<pasta_do_coletor>/database_4h, nao no diretorio de trabalho."
        )
    return float(np.median(amostras))


# resolve a pasta: absoluta, ou relativa ao proprio script (onde o coletor grava)
if not os.path.isabs(dados_base):
    _cand = os.path.join(os.path.dirname(os.path.abspath(__file__)), dados_base)
    if os.path.isdir(_cand):
        dados_base = _cand

_n_arq = (len([f for f in os.listdir(dados_base)
               if f.endswith((".parquet", ".xlsx"))]) if os.path.isdir(dados_base) else 0)
print(f"BASE: {os.path.abspath(dados_base)}")
print(f"  existe: {os.path.isdir(dados_base)} | arquivos de dados: {_n_arq}")
_faltando = [t for t in TICKERS
             if not any(os.path.exists(os.path.join(dados_base, f"{t}{e}"))
                        for e in (".parquet", ".xlsx"))]
if _faltando:
    print(f"  AUSENTES ({len(_faltando)} de {len(TICKERS)}): {_faltando[:10]}"
          f"{' ...' if len(_faltando) > 10 else ''}")

if BARRAS_POR_DIA is None:
    BARRAS_POR_DIA = _detectar_barras_por_dia(TICKERS, dados_base)
BARRAS_ANO = BARRAS_POR_DIA * PREGOES_ANO
if barras_minimas_hilo is None:
    barras_minimas_hilo = int(round(BARRAS_ANO))
# janelas moveis: declaradas em dias, aplicadas em barras
janelas_carteira = [max(1, int(round(w * BARRAS_POR_DIA))) for w in janelas_dias]
# hilos fixos: pregoes -> barras, respeitando o teto da grade
hilos_fixos_janelas = sorted({
    min(hilo_max, max(hilo_min, int(round(p * BARRAS_POR_DIA))))
    for p in hilos_fixos_pregoes})
ret_liq_por_hilo = {h: {} for h in hilos_fixos_janelas}
series_longonly = {h: {} for h in hilos_fixos_janelas}

print(f"TIMEFRAME 4H — barras/pregao detectadas: {BARRAS_POR_DIA:.2f} "
      f"-> BARRAS_ANO = {BARRAS_ANO:.0f}")
print(f"  janelas (dias -> barras): "
      + ", ".join(f"{d}d={b}b" for d, b in zip(janelas_dias, janelas_carteira)))
print(f"  hilos fixos (pregoes -> barras): "
      + ", ".join(f"{p:.0f}p={b}b" for p, b in zip(sorted(hilos_fixos_pregoes),
                                                   hilos_fixos_janelas)))
print(f"  hilo {hilo_min}-{hilo_max} barras = "
      f"{hilo_min/BARRAS_POR_DIA:.0f}-{hilo_max/BARRAS_POR_DIA:.0f} pregoes de lookback")
print(f"  dias_minimos_selecao = {dias_minimos_selecao} barras = "
      f"{dias_minimos_selecao/BARRAS_POR_DIA:.0f} pregoes")
...
for ticker in TICKERS:
    ...
    caminho_arquivo = os.path.join(dados_base, f"{ticker}.parquet")
    ...
    try:
        ...
        if ultimo_df_bt is not None and not ultimo_df_bt.empty:
            ...
            holding_aa = annualize_return(holding_total, len(ultimo_df_bt), BARRAS_ANO)
        else:
...
def _sharpe_deflacionado(sharpe_aa, n_obs, n_trials, skew, kurt):
    ...
    sr = sharpe_aa / np.sqrt(BARRAS_ANO)              # Sharpe por barra
    if n_trials == 1:
    ...
    if den <= 0:
        return float(sr0 * np.sqrt(BARRAS_ANO)), np.nan
    z = (sr - sr0) * np.sqrt(n_obs - 1.0) / np.sqrt(den)
    dsr = float(stats.norm.cdf(z))               # P(Sharpe verdadeiro > sr0)
    return float(sr0 * np.sqrt(BARRAS_ANO)), float(1.0 - dsr)
...
def montar_aba_pvalor(wf_curvas_df, n_trials=None, n_boot=2000):
    ...
    for ativo, g in df.groupby("Ativo"):
        ...
        sd = x.std(ddof=1)
        sharpe_aa = (x.mean() / sd * np.sqrt(BARRAS_ANO)) if sd > 0 else np.nan
        ...
        linhas.append({
            ...
            "Vol_AA": float(sd * np.sqrt(BARRAS_ANO)),
            "Sharpe_AA": sharpe_aa,
            "Assimetria": sk,
            "Curtose": ku,
            "t_Padrao": t_padrao,
    ...
    if len(port) >= 100:
        xp = port.values
        sdp = xp.std(ddof=1)
        sh_p = (xp.mean() / sdp * np.sqrt(BARRAS_ANO)) if sdp > 0 else np.nan
        ...
        agregados.append({
            ...
            "N_Barras": len(xp),
            "Anos": len(xp) / BARRAS_ANO,
            "Ret_Medio_Diario": float(xp.mean()),
            "Vol_AA": float(sdp * np.sqrt(BARRAS_ANO)),
            "Sharpe_AA": sh_p,
            "Assimetria": skp,
            "Curtose": kup,
            "t_Padrao": float(t_ip),
        ...
        for ini in range(a0, a1 + 1, anos_por_bloco_pvalor):
            ...
            agregados.append({
                ...
                "Ativo": f"{piv.shape[1]} ativos",
                "N_Barras": len(seg),
                "Anos": len(seg) / BARRAS_ANO,
                "Ret_Medio_Diario": float(seg.mean()),
                "Vol_AA": float(sseg * np.sqrt(BARRAS_ANO)),
                "Sharpe_AA": float(seg.mean() / sseg * np.sqrt(BARRAS_ANO)),
                "t_Padrao": float(t_s),
...
def montar_carteira_wf(wf_curvas_df):
    ...
    if len(dif) > 30 and dif.std(ddof=1) > 0:
        ...
        linha_dif = {
            ...
            "N_Barras": len(dif),
            "Anos": len(dif) / BARRAS_ANO,
            "Rent_AA_Geometrica": float(dif.mean() * BARRAS_ANO * 100.0),
            "Vol_AA": sd_d * np.sqrt(BARRAS_ANO) * 100.0,
            "t_Padrao": t_d,
...
def _metricas_carteira(ret_serie, curva, rotulo):
    ...
    geo = float(curva.iloc[-1] / montante_inicial) ** (BARRAS_ANO / n) - 1.0 if n > 0 else np.nan
    sh = float(ret_serie.mean() / sd * np.sqrt(BARRAS_ANO)) if sd > 0 else np.nan
